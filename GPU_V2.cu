...
__global__ void lin_solve_kernel(float *x, float *x0, float a, float c) {
    ...
    __shared__ float sharedX[(N+2)*3];
    ...
    if(j >= 1 && j <= N) {
        ...
        sharedX[IX(threadIdx.x, 0)] = x[IX(i, j - 1)];
        sharedX[IX(threadIdx.x, 1)] = x[IX(i, j)];
        sharedX[IX(threadIdx.x, 2)] = x[IX(i, j + 1)];

        __syncthreads();
        if (i >= 1 && i <= N) {
            // Perform the relaxation step
            sharedX[IX(threadIdx.x, 1)] =
                    (x0[IX(i, j)] + a * (sharedX[IX(threadIdx.x -1, 1)] + sharedX[IX(threadIdx.x + 1, 1)] + sharedX[IX(threadIdx.x, 0)] + sharedX[IX(threadIdx.x, 2)])) / c;
            x[IX(i, j)] = sharedX[IX(threadIdx.x, 1)];
        }
    }
}
...
__global__ void project_kernel_div(float * u, float * v, float * p, float * div){
    ...
    __shared__ float sharedU[(N+2)*3];
    __shared__ float sharedV[(N+2)*3];
    ...
    if(j >= 1 && j <= N) {
        ...
        sharedU[IX(threadIdx.x, 0)] = u[IX(i, j - 1)];
        sharedU[IX(threadIdx.x, 1)] = u[IX(i, j)];
        sharedU[IX(threadIdx.x, 2)] = u[IX(i, j + 1)];

        sharedV[IX(threadIdx.x, 0)] = v[IX(i, j - 1)];
        sharedV[IX(threadIdx.x, 1)] = v[IX(i, j)];
        sharedV[IX(threadIdx.x, 2)] = v[IX(i, j + 1)];

        __syncthreads();
        if (i >= 1 && i <= N) {
            div[IX(i, j)] = -0.5f * h * (sharedU[IX(threadIdx.x +1, 1)] - sharedU[IX(threadIdx.x -1, 1)] + sharedV[IX(threadIdx.x, 2)] - sharedV[IX(threadIdx.x, 0)]);
            p[IX(i, j)] = 0;
        }
    }
}
...
__global__ void project_kernel_UV(float * u, float * v, float * p){
    ...
    __shared__ float sharedP[(N+2)*3];
    ...
    if(j >= 1 && j <= N) {
        ...
        sharedP[IX(threadIdx.x, 0)] = p[IX(i, j - 1)];
        sharedP[IX(threadIdx.x, 1)] = p[IX(i, j)];
        sharedP[IX(threadIdx.x, 2)] = p[IX(i, j + 1)];

        __syncthreads();
        if (i >= 1 && i <= N) {
            u[IX(i, j)] -= 0.5f * N * (sharedP[IX(threadIdx.x + 1, 1)] - sharedP[IX(threadIdx.x - 1, 1)]);
            v[IX(i, j)] -= 0.5f * N * (sharedP[IX(threadIdx.x, 2)] - sharedP[IX(threadIdx.x, 0)]);
        }
    }
}
...
__global__ void draw_density_kernel(float* dens, int* colors){
    ...
    __shared__ float sharedDens[(N+2)*3];
    ...
    if(j >= 1 && j <= N) {
        ...
        sharedDens[IX(threadIdx.x,0)] = dens[IX(i, j - 1)];
        sharedDens[IX(threadIdx.x, 1)] = dens[IX(i, j)];
        sharedDens[IX(threadIdx.x, 2)] = dens[IX(i, j + 1)];

        __syncthreads();

        if (i >= 1 && i <= N) {
            float avg_dens = (sharedDens[IX(threadIdx.x, 1)] + sharedDens[IX(threadIdx.x, 2)] + sharedDens[IX(threadIdx.x + 1, 1)] + sharedDens[IX(threadIdx.x + 1, 2)]) / 4;
...
int main() {
    ...
    myFile << "number of threads" << ";" << "calculate time" <<"\n";
